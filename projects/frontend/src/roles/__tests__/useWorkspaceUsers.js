...
describe('useWorkspaceUsers', () => {
    it('should return loading if any of the requests is still pending', async () => {
        ...
        const {result, waitForNextUpdate} = renderHook(() => useWorkspaceUsers(workspacename, keycloakAPI));
    ...
    describe('user lists', () => {
        ...
        it('should add authorizations for all other roles', async () => {
            ...
            expect(result.current.users).toEqual(expect.arrayContaining([
                {id: 1, name: 'John', authorizations: {user: true, coordinator: false, datasteward: false, sparqluser: false}},
                {id: 2, name: 'Jane', authorizations: {user: true, coordinator: true, datasteward: false, sparqluser: true}},
                {id: 3, name: 'Simon', authorizations: {user: true, coordinator: true, datasteward: true, sparqluser: false}},
                {id: 4, name: 'Mary', authorizations: {user: true, coordinator: false, datasteward: false, sparqluser: true}},
            ]));
        });

        it('refresh the user list if asked for', async () => {
            let coordinators;
